...
export default function RequestsPage() {
  ...
  const orderSelectionOptions = useMemo(() => {
    ...
    if (selectedOrderId) {
      const selected = (confirmedOrders ?? []).find((order) => order.orderId === selectedOrderId);
      if (selected) {
        options.push(selected);
        seen.add(selected.orderId);
      }
    }
    ...
    return options;
  }, [confirmedOrders, filteredConfirmedOrders, selectedOrderId]);
